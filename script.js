        ...
        (async function() {
          try {
            ...
            const response = await fetch('https://api.zappy5.com/api/email/contact-form', {
              ...
              body: JSON.stringify({
                websiteId: '41bf8c08-a846-436d-88b0-fc4660915e63',
                name: data.name || data.fullName || data.fullname || '',
                email: data.email || '',
                subject: data.subject || data.topic || 'Contact Form Submission',
            ...
            if (response.ok) {
              ...
              console.log('⚠️ Zappy: Server returned error status', response.status);
            }
          } catch (error) {
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "צמפיונשיפ לוגיסטיקה משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
